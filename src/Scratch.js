...
class Scratch extends Sprite {
	...
	getMediaPane(app:Scratch, type):MediaPane {
		return new MediaPane(app, type);
	}

	getScratchStage():ScratchStage {
		return new ScratchStage();
	}

	getPaletteBuilder():PaletteBuilder {
		return new PaletteBuilder(this);
	}

	uncaughtErrorHandler(event = new UncaughtErrorEvent()) {
	...
	viewedObj() = new ScratchObj() {
		return viewedObject;
	}

	stageObj() = new ScratchStage() {
		return stagePane;
	}
	...
	highlightSprites(sprites = new Array()) {
	...
	selectSound(snd = new ScratchSound()) {
